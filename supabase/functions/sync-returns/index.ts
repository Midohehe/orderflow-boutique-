// Syncs customer returns receipts (استلام المرتجعات) from the shipping
// company (Turbo Express). Same as sync-settlements but with typeCode RTRN.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const rows = allPayments.map((p) => ({
      ...
      external_id: p.id,
      code: p.code,
      return_date: p.date ? new Date(p.date.replace(" ", "T") + "Z").toISOString() : null,
